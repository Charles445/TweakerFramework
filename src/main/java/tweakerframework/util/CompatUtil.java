...
public class CompatUtil
{
	...
	public static void wrapSpecificHandler(String name, Consumer<IEventListener> constructor, String clazzName, String methodName) throws Exception
	{
		...
		{
			TweakerFramework.logger.info("Registering "+name+" to the event bus");
	...
	public static void subscribeEventManually(Class<?> clazz, Object thiz, Method thiz_toCall) throws Exception
	{
		...
		TweakerFramework.logger.info("Registered "+thiz.getClass().getName()+" "+thiz_toCall.getName()+" to the event bus");
	...
	public static Object getModInstance(String modid)
	{
		...
		TweakerFramework.logger.warn("Asked to find mod instance "+modid+", but could not find it!");
			...
			for(Map.Entry<Object, ArrayList<IEventListener>> listener_entry : listeners.entrySet())
			{
				...
				if (handlerName.equals(name))
				{
					...
					if(specific==null)
					{
						...
						TweakerFramework.logger.info("Found and removed "+name+" from the event bus");
					...
					else
					{
						...
						while(elIterator.hasNext())
						{
							...
							if(eventListener.toString().contains(specific)) //TODO consider accuracy, or not, as this allows for desc usage as-is
							{
								...
								TweakerFramework.logger.info("Found and removed "+name+" IEventListener "+specific+" from the event bus");
	...
	public static <W extends WorldGeneratorWrapper> W tryWrapWorldGenerator(final W wrapperIn, Class<?> targetGeneratorClazz) throws Exception
	{
		...
		try
		{
			...
			{
				TweakerFramework.logger.error("worldGenerators was null in tryWrapWorldGenerator");
			...
			{
				TweakerFramework.logger.error("worldGeneratorIndex was null in tryWrapWorldGenerator");
			...
			{
				TweakerFramework.logger.error("Could not find world generator of class: "+targetGeneratorClazz.getName());
				ErrorUtil.logSilent("tryWrapWorldGenerator worldGenerators null");
				return null;
			}
			...
			if(weight == null)
			{
				TweakerFramework.logger.error("Found world generator of class but weight was null: "+targetGeneratorClazz.getName());
			...
			{
				TweakerFramework.logger.error("Failed to remove in worldGenerators with class: "+targetGeneratorClazz.getName());
				return null;
			}
			
			if(worldGeneratorIndex.remove(foundGenerator) == null)
			{
				TweakerFramework.logger.error("Failed to remove in worldGeneratorIndex with class: "+targetGeneratorClazz.getName());
				return null;
			}
			...
			TweakerFramework.logger.info("Wrapped IWorldGenerator of class "+targetGeneratorClazz.getName());
